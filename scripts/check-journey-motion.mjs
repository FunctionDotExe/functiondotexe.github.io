...
function mount(initialY = 9000, reduceMotion = false) {
  let time = 0;
  let serial = 0;
  let cleanup;
  ...
  const media = new Map();
  const context = {
    exports: {}, scrollY: initialY, innerWidth: 1440, innerHeight: 900,
  ...
  function element(name, top = 0, height = 900, width = 1440) {
    ...
    return {
      ...
      getBoundingClientRect: () => ({ top: top - context.scrollY, height, width }),
  ...
  const worldNodes = new Map([
    ...
    [".journey-world__continuum-plate", element("plate", 0, 3025, 1600)],
  ...
  context.document = {
    ...
    querySelector: (selector) => new Map([[".journey-world", world], [".descent-threshold", threshold], [".journey__story", story]]).get(selector),
