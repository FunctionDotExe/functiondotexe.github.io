          ...
          {SUMMIT_CONTENT.landmarks.map((project, index) => (
            <article className={`waypoint waypoint--${project.visual}`} id={`project-${project.visual}`} aria-labelledby={`title-${project.visual}`} key={project.title}>
              ...
              <div className="waypoint__copy">
                ...
                <div className="project-actions"><ProjectInspectButton />{project.visual === "signal" && <a className="text-link" href={RESUME.youtubeUrl} target="_blank" rel="noreferrer"><Play size={16} aria-hidden="true" />Watch demo<span className="sr-only"> on YouTube (opens in a new tab)</span></a>}</div>
              </div>
              ...
              <figure className={`project-media project-media--${project.visual}`}>
                ...
                <ProjectImageButton>
                <span className="project-media__canvas">
